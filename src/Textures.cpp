...
textures::textures(const aiScene *scene, std::string sPath){
	...
	for (unsigned int m=0; m<scene->mNumMaterials; m++)
	{
		...
		aiString* path = new aiString(sPath);	// filename

		while (texFound == AI_SUCCESS)
		{
			cout << texIndex <<": found" << endl;
			texFound = scene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE, texIndex, path);
			textureIdMap[path->data] = NULL; //fill map with textures, pointers still NULL yet
			cout << "path data is  "<<  path->data << " adding" << endl;
			texIndex++;
		}
	}
...
bool textures::bindTextures(std::string basepath)
{
	...
	for (int i=0; i<numTextures/* && success*/; i++)
	{
		if (i != 0){
			...
			cout<< "looking at "<<fileloc<<" with id "<<textureIds[i]<<endl;
